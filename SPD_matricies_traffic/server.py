...
import asyncio, json, struct, sys, threading, time, warnings
...
import cupy as cp
from fleet_sim   import Fleet, TrafficField, Incident, CITY_SIZE
from enkf_solver import FleetAssimilator, _DEFAULT_GAMMA, _DEFAULT_REG
...
SOLVER_MODE = 1 if USE_OOC else 0   # 0=standard, 1=ooc-ram
...
class SimState:
    def __init__(self):
        ...
        self.trail_indices     = _rng.choice(N_VEHICLES, size=N_TRAIL,
...
def _run_startup_benchmark():
    ...
    print(f"  Matrix: {N_VEHICLES:,}×{N_VEHICLES:,}  "
          f"FP64={fp64_gb:.1f}GB  FP32={fp32_gb:.1f}GB  VRAM={vram_gb:.1f}GB")
    print(f"  Solver: {'OOC-RAM' if USE_OOC else 'standard MPDOK'}")
    ...
    try:
        ...
        state.cupy_oom = True
        print(f"  [CuPy probe] ✗ OutOfMemoryError — "
    ...
    print(f"  [SciPy estimate] ~{_fmt_time(est)} "
          f"(O(N³) from N={N_ref:,} baseline)")
    ...
    if USE_OOC:
        ...
        ooc.build(coords, gamma=_DEFAULT_GAMMA, reg=_DEFAULT_REG,
                  store='ram', verbose=False)
        ...
        with warnings.catch_warnings():
            ...
            x = ooc.solve(b, tol=OOC_TOL,
                          maxiter_outer=OOC_OUTER, restart=OOC_RESTART,
                          verbose=True)
        ...
        print(f"  [MPDOK std] {mpdok_ms:.0f}ms  SciPy {scipy_ms:.0f}ms  "
    ...
    if USE_OOC:
        ...
        print(f"  [MPDOK OOC] cycle {_fmt_time(cycle_s)}  "
...
def solve_loop():
    ...
    while state.running:
        ...
        if USE_OOC:
            print(f"  [assim {state.assim_step}] "
                  f"build={build_s:.1f}s  solve={solve_s:.1f}s  "
                  f"cycle={cycle_s:.1f}s", flush=True)
...
def pack_frame() -> bytes:
    with state.lock:
        ...
        incidents = [(i.x, i.y, i.strength(state.fleet.t))
                     for i in state.field.incidents]
    ...
    inc_arr = np.array(incidents, dtype=np.float32).reshape(-1, 3) if incidents \
              else np.zeros((0, 3), dtype=np.float32)
    inc_header = struct.pack("<i", len(incidents))

    return (header + tf.tobytes() + rf.tobytes() + pos.ravel().tobytes()
            + yv.tobytes() + trail_header + trail_arr.tobytes()
            + inc_header + inc_arr.tobytes())
...
async def ws_endpoint(ws: WebSocket):
    ...
    last_frame = -1
    caps = {
        "N": N_VEHICLES, "city_size": CITY_SIZE, "nx": NX, "ny": NY,
        "solver_mode": SOLVER_MODE, "ooc_outer": OOC_OUTER,
        ...
        while True:
            try:
                ...
                with state.lock:
                    ...
                    if "incident" in cmd:
                        d = cmd["incident"]
                        inc = Incident(x=float(d["sx"]), y=float(d["sy"]),
                                       t_start=state.fleet.t)
...
async def startup():
    fp32_gb = N_VEHICLES ** 2 * 4 / 1e9
    print("=" * 62)
    print(f"  SPD Fleet Tracker — MPDOK {'OOC-RAM' if USE_OOC else 'Standard'}")
